...
// Undo/redo: 8 levels of in-memory state snapshots (session-only).
...
function doRender(entries) {
  ...
  // Compute overlaps only when the warning UI is active — computeAllOverlaps
  // is O(n²) and unnecessary when the user has turned warnings off.
  const warnings = showWarnings
    ? { overlappingIds: computeAllOverlaps(entries), entries, showTooltips }
    : null;
