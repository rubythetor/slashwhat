...
describe('buffer cap', () => {
  ...
  it('respects default cap of 50', () => {
    for (let i = 0; i < 55; i++) mgr.push(`s${i}`);
    // Only the last 50 should remain (s5..s54)
    let count = 0;
    let current = 'final';
    while (mgr.undo(current) !== null) {
      current = mgr.undo(current) ?? current;
      count++;
    }
    // Re-count properly: undo returns non-null exactly 50 times
    const fresh = createUndoManager(50);
    for (let i = 0; i < 55; i++) fresh.push(`s${i}`);
    let undoCount = 0;
    let cur = 'end';
    while (true) {
