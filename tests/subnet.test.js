...
describe('Subnet family flags', () => {
  it('reports IPv4 and IPv6 flags for IPv4 subnets', () => {
    const s = Subnet.parse('10.0.0.0/8');
    assert.equal(s.isIPv4, true);
    assert.equal(s.isIPv6, false);
  });

  it('reports IPv4 and IPv6 flags for IPv6 subnets', () => {
    const s = Subnet.parse('2001:db8::/32');
    assert.equal(s.isIPv4, false);
    assert.equal(s.isIPv6, true);
  });
});
